<template>
  <div class="min-h-screen bg-dark-950 py-12">
    <div class="container-narrow">
      <h1 class="text-3xl font-bold text-dark-100 mb-8">Create New Post</h1>
        ...
        <div class="card-body">
          <form @submit.prevent="handleSubmit">
            ...
            <div class="mt-6 flex space-x-4">
              ...
              <router-link to="/posts" class="btn btn-outline btn-lg"> Cancel </router-link>
            </div>
          </form>
        </div>
      </BaseCard>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const newPost = await postsStore.createPost(formData as unknown as PostCreateUpdate)
    ...
    if (err.response?.data) {
      ...
      if (!errors.title && !errors.content && !errors.category && !errors.image) {
        errors.general = (data.detail as string) || 'Failed to create post'
      }
    } else {
...
onMounted(async () => {
  try {
    ...
    } else if (typeof cats === 'object' && cats !== null && 'results' in cats && Array.isArray(cats.results)) {
